    ...
    db('navire_data').innerJoin('navire', 'navire_data.navire_id', '=', 'navire.id')
        ...
        .then(data => {
            if (data.length != 0) {
                ...
                db('dum').select('numero_dum','quantite_dum','quantite_dum_restante','quantite_dum_rest_estimee','status').where(newdict).orderBy('numero_dum','asc')
                    .then(dataset => {
                        console.log('dataset',dataset)
                        if (dataset.length === 0) {
                            console.log('no data in the DB dum for', newdict)
                            res.json({ dataset})
                        }
                        else {
                            res.json({ dataset})
